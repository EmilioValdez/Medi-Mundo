...
export default function BlogPostPage() {
  ...
  if (loading) return (
    <div className="mx-auto max-w-3xl px-4 py-16 sm:px-6 lg:px-8 animate-pulse space-y-4">
      <div className="h-3 bg-gray-200 rounded w-32" />
      <div className="h-8 bg-gray-200 rounded w-full" />
      <div className="h-8 bg-gray-200 rounded w-3/4" />
      <div className="space-y-2 mt-6">
        {[...Array(8)].map((_, i) => <div key={i} className="h-3 bg-gray-100 rounded w-full" />)}
      </div>
    </div>
  );
  ...
  return (
    <>
      <Helmet>
        <title>{post.meta_title || post.titulo} | MediMundo Querétaro</title>
        <meta name="description" content={post.meta_description || post.resumen} />
        <link rel="canonical" href={`https://medimundo.mx/blog/${post.slug}`} />
        <meta property="og:title" content={post.meta_title || post.titulo} />
        <meta property="og:description" content={post.meta_description || post.resumen} />
      ...
      <div className="mx-auto max-w-3xl px-4 py-12 sm:px-6 lg:px-8">
        ...
        <nav className="text-sm text-gray-400 mb-8 flex items-center gap-1.5 flex-wrap">
          ...
          <span className="text-gray-700 truncate max-w-xs">
            {post.titulo?.slice(0, 55)}{post.titulo?.length > 55 ? '…' : ''}
          </span>
        </nav>
