...
export default function EquipmentCard({
  ...
  const waMsg = mode === "sale"
    ? waLink(`Hola, me interesa comprar: *${item.name}*. ¿Está disponible y cuál es el precio?`)
    : waLink(`Hola, me interesa rentar: *${item.name}*. ¿Está disponible y cuál es el precio?`);
  ...
  return (
    <div className="group flex flex-col overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm transition-all hover:shadow-lg">
      ...
      <div className="flex flex-1 flex-col px-6 pb-6 pt-4">
        ...
        {mode === "sale" ? (
          item.price_sale != null && item.price_sale > 0 ? (
            <div className="mt-3">
              <p className="text-4xl font-black text-gray-900 leading-none">{formatMXN(item.price_sale)}</p>
              <p className="mt-1 text-sm text-gray-400">Precio de venta</p>
            </div>
          ) : (
            <p className="mt-2 text-sm text-gray-400">Consultar precio</p>
          )
        ) : item.price_monthly != null && item.price_monthly > 0 ? (
          <div className="mt-3">
        ...
        >
          ...
          {mode === "sale" ? "Comprar este producto" : "Rentar este producto"}
        </a>
