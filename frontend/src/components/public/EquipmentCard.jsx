...
function getPlaceholder(categoryName) {
  if (!categoryName) return { emoji: '🏥', bg: 'bg-gray-50' };
...
/* Split "Cama Hospitalaria Manual" → ["Cama", "Hospitalaria Manual"] */
...
export default function EquipmentCard({ item, showPrices = false, specs = null }) {
  ...
  const [namePrefix, nameSuffix] = splitName(item.name);
  ...
  return (
    <div className="group flex flex-col overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm transition-all hover:shadow-lg">
      ...
      <div className={`relative flex items-center justify-center px-8 pt-8 pb-4 ${imageUrl ? 'bg-white' : placeholder.bg}`} style={{ minHeight: '220px' }}>
        {imageUrl ? (
          <img
            src={imageUrl}
        ...
        <div className={`${imageUrl ? 'hidden' : 'flex'} flex-col items-center justify-center h-48`}>
      ...
      <div className="flex flex-1 flex-col px-6 pb-6 pt-4">
        ...
        {/* Name: first word black, rest blue */}
        <h3 className="text-xl font-bold leading-snug">
          <span className="text-gray-900">{namePrefix} </span>
          {nameSuffix && <span className="text-primary-600">{nameSuffix}</span>}
        </h3>
        ...
        <Link
          to={`/equipo/${item.id}`}
          className="mt-2 text-center text-sm text-gray-400 hover:text-primary-600 transition-colors"
        >
          Ver detalles
        </Link>
